function [weightsEE, weightsEI, weightsIE, weightsII] = create_EIIE_topology(EneuronNum,numClusters, PARAMS)
    ...
    if nargin < 3
    % in this scenario all factors are defined normally, no
    % "inverse" ratios
    % Everything was 2.5 in the "old" dFFN
    PARAMS.factorEI  = 2.6;%2.5;
    PARAMS.factorIE  = 2.6;%2.5;
    PARAMS.pfactorEI = 2.6;%.5;%2.5;
    PARAMS.pfactorIE = 2.6;%.5;%2.5;
    
    % II coupling
    PARAMS.factorII  = 2.6;
    PARAMS.pfactorII = 2.6;%2.5;
    ...
    wIIsub = PARAMS.wII/(1/numClusters+(1-1/numClusters)/PARAMS.factorII);
    PARAMS.wII = wIIsub/PARAMS.factorII;
    pIIsub = PARAMS.pII/(1/numClusters+(1-1/numClusters)/PARAMS.pfactorII);
    PARAMS.pII = pIIsub/PARAMS.pfactorII;


    %Weight matrix of inhibitory to excitatory LIF units
    weightsEI = random('binom',1,PARAMS.pEI,[EneuronNum,IneuronNum]);                                        
    weightsEI = PARAMS.wEI.* weightsEI;

    %Weight matrix of excitatory to inhibitory cells
    ...
    % II adjustments
    for i = 1:numClusters
        weightsIIsub = random('binom',1,pIIsub,[IneuronNum/numClusters, IneuronNum/numClusters]);
        weightsIIsub = wIIsub.* weightsIIsub;              
        kk = (i-1)*IneuronNum/numClusters+1:i*IneuronNum/numClusters;
        jj = (i-1)*IneuronNum/numClusters+1:i*IneuronNum/numClusters;
        weightsII(jj,kk) = weightsIIsub;
    end
    weightsII = circshift(weightsII,[0 IneuronNum/numClusters]);
